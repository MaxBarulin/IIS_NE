...
from qtpy.QtCore import Signal, QRect, QLine, Qt, QPointF
...
class QDMGraphicsScene(QGraphicsScene):
    ...
    def initAssets(self):
        ...
        self._color_dark = QColor("#dfdfdf")
    ...
    def snap_to_grid(self, point):
        ...
        return QPointF(
            round(point.x() / gridSize) * gridSize,
            round(point.y() / gridSize) * gridSize
        )

    def mouseMoveEvent(self, event):
        """Обработка события перемещения мыши с привязкой к сетке"""
        if event.buttons() == Qt.LeftButton:
            for item in self.selectedItems():
                if hasattr(item, 'node'):
                    # Привязка позиции ноды к сетке
                    pos = self.snap_to_grid(item.pos())
                    item.setPos(pos)
        super().mouseMoveEvent(event)

    def mouseReleaseEvent(self, event):
        """Обработка события отпускания мыши с привязкой к сетке"""
        if event.button() == Qt.LeftButton:
            for item in self.selectedItems():
                if hasattr(item, 'node'):
                    # Привязка позиции ноды к сетке при отпускании мыши
                    pos = self.snap_to_grid(item.pos())
                    item.setPos(pos)
